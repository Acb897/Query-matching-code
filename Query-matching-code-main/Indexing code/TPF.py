...
def execute_sparql_query(query, include_types=False):
    """
    include_types=False (default, unchanged): each binding is flattened to
    its plain string value, as used by FindBGPPriority for join matching.

    include_types=True: each binding is kept as the full SPARQL-JSON dict
    ({"type": "uri"|"bnode"|"literal"|..., "value": ...}) so callers can
    distinguish a URI/IRI term from a blank node or literal without
    guessing from the string (e.g. run_query_strict / TPFAdapter, which
    must not mistake a blank-node class for a resolvable class IRI).
    """
...
def run_query_strict(query, endpoints, base_named_graph="urn:tpf:temp"):
    ...
    # include_types=True so downstream adapters (TPFAdapter) can tell a
    # URI/IRI term apart from a blank node or literal instead of guessing
    # from the string value (see adapters/tpf_adapter.py).
